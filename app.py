...
@app.route('/api/<username>/todos', methods=['GET','POST'])
@app.route('/api/<username>/todos/<int:id>', methods=['GET','PUT','DELETE'])
def todos(username,id=None):
    if request.method=='GET':
        if id is not None:
            todo = Todo.query.filter_by(id=id, username=username).first()
            if todo:
                return jsonify(todo.serialize()),200
            else:
                return jsonify({'msg':'Not Found'}),404
        else:            
            todos = Todo.query.filter_by(username=username).all() #toma todos los contactos
    ...
    if request.method == 'POST':
        ...
        '''Under to this comment, we comment the if because we can't validate a '''
